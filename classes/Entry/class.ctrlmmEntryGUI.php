 ...
 *
 * @ilCtrl_IsCalledBy ctrlmmEntryGUI: ilAdministrationGUI, ilPersonalDesktopGUI, ilRepositoryGUI
 */
class ctrlmmEntryGUI {
	 ...
	 */
	function __construct(ctrlmmEntry $entry, $parent_gui = NULL) {
		global $ilCtrl, $tpl;
		/**
		 * @var $ilCtrl ilCtrl
		 */
	...
	public function renderEntry() {
		...
		$this->html = $this->pl->getTemplate('tpl.menu_entry.html', true, true);
		...
		$this->html->setVariable('STATE', ($this->entry->isActive() ? $this->pl->getConfigObject()
			->getCssActive() : $this->pl->getConfigObject()->getCssInactive()));
	...
	public static function includeAllEntryTypeGUIs() {
		foreach (scandir(dirname(__FILE__) . '/EntryTypes/') as $file) {
			if (strpos($file, 'GUI.php')) {
				include_once(dirname(__FILE__) . '/EntryTypes/' . $file);
			}
		}
	}
}
