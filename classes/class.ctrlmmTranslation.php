...
class ctrlmmTranslation {
	...
	public static function _getInstanceForLanguageKey($entry_id, $language_key) {
		global $ilDB;
		// Existing Object
		$set = $ilDB->query('SELECT id FROM ' . self::TABLE_NAME . ' ' . ' WHERE language_key = '
			. $ilDB->quote($language_key, 'text') . ' AND entry_id = ' . $ilDB->quote($entry_id, 'integer'));
	...
	public static function _getAllTranslationsAsArray($entry_id) {
		global $ilDB;
		$return = array();
		$set = $ilDB->query('SELECT language_key, title FROM ' . self::TABLE_NAME . ' WHERE entry_id = '
			. $ilDB->quote($entry_id, 'integer'));
		while ($rec = $ilDB->fetchObject($set)) {
	...
	public function _getAllInstancesForEntryId($entry_id) {
		global $ilDB;
		$return = array();
		$set = $ilDB->query('SELECT id FROM ' . self::TABLE_NAME . ' WHERE entry_id = '
			. $ilDB->quote($entry_id, 'integer'));
		while ($rec = $ilDB->fetchObject($set)) {
			$return[] = new self($rec->id);
		}
