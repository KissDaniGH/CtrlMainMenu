...
class ctrlmmEntryCtrl extends ctrlmmEntry {
	...
	public function isActive() {
		foreach ($this->ctrl->getCallHistory() as $class) {
			if (in_array($class['class'], explode(',', $this->getGuiClass())) && $class['class'] != 'ilRouterGUI') {
				return true;
			}
		}
	}
		...
		if (ctrlmmMenu::isOldILIAS()) {
			...
			try {
				...
				if (self::DEBUG AND $ilUser->getId() == 6) {
					ilUtil::sendFailure('ctrlmmEntryCtrl::getLink() : ERROR parsing ilCtrl-Link (' . $e->getMessage()
						. ')', true);
				}
			}
		}
